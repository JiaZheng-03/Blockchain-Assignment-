...
export default function Sidebar({ onClose }) {
  ...
  const { isShipper, isArbitrator, profile, roleLabel } = useProfile();
  ...
  return (
    <aside className="sidebar" id="workspace-navigation">
          ...
          <div className="nav-group" key={group.label}>
            ...
            {group.items.map(([to, label, icon]) => (
              <NavLink end={to === '/'} key={to} to={to} onClick={onClose}>
                ...
                {to === '/create-agreement' && authenticatedShipper && <span className="nav-mini-label">NEW</span>}
              </NavLink>
            ))}
          </div>
        ))}
      ...
      <div className="sidebar-bottom">
        ...
        <Link className="sidebar-account" to={isAuthenticated ? '/profile' : '/login'} onClick={onClose}>
          <span className="account-avatar">{authenticatedProfile?.name?.slice(0, 2).toUpperCase() || <Icon name="user" size={18} />}</span>
          <span>
            <strong>{authenticatedProfile?.name || (isAuthenticated ? formatAddress(account) : 'Your workspace')}</strong>
            <small>{isAuthenticated ? roleLabel : 'Login with your wallet'}</small>
          </span>
